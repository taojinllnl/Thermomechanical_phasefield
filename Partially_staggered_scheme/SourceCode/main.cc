 ...
 * crack problem. The entire problem is decoupled into two subproblems, the damage
 * (phase-field) problem and the thermomechanical problem (u and T):
 ...
 * 5. This code implements a hybrid staggered approach. The displacement and temperature
 *    are updated together. The phase-field is updated separately. The phase-field
 ...
 * 8. The displacement and temperature are solved together using Newton method, and the
 *    phase-field problem itself is linear.
 */
...
/*
  ...
  usr_sparseILU_preconditioner::usr_sparseILU_preconditioner(const SparseILU<double> & ILU_factorization_u,
							     const SparseILU<double> & ILU_factorization_t)
  : m_ILU_factorization_u(& ILU_factorization_u)
...
*/
  ...
  namespace Parameters
  {
    ...
    void Scenario::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Scenario");
      {
        ...
        prm.declare_entry("Nonlinear solver type",
                          ...
                          "Type of solver used to solve the thermomechanical (ut) nonlinear system");
        ...
        prm.declare_entry("GMRES ut tolerance",
			  "1.0e-9",
			  Patterns::Double(0.0),
			  "If Iterative is selected as linear solver, the tolerance of GMRES for ut-subproblem");

        prm.declare_entry("Ut Newton tolerance",
        	          "1.0e-9",
        	          Patterns::Double(0.0),
    ...
    void NonlinearSolver::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Nonlinear solver");
      {
        ...
        prm.declare_entry("Max iterations UT",
                          "20",
                          Patterns::Integer(0),
                          "Number of Newton iterations allowed for the UT subproblem");

        prm.declare_entry("Tolerance ut residual",
                          "1.0e-9",
                          Patterns::Double(0.0),
        ...
        prm.declare_entry("Tolerance ut increment",
                          "1.0e-9",
                          Patterns::Double(0.0),
	...
	m_kappa_d = 1.0 * m_kappa_0;
      ...
      SymmetricTensor<2, dim> strain_positive_temperature
                            = usr_spectrum_decomposition::
                              p_strain_positive_p_temperature(eigenvalues,
      				   		              eigenvectors,
      							      m_alpha);
      ...
      SymmetricTensor<2, dim> strain_negative_temperature
                            = usr_spectrum_decomposition::
			      p_strain_negative_p_temperature(eigenvalues,
  						              eigenvectors,
  							      m_alpha);
      ...
      const double trace_strain_negative_temperature
                 = usr_spectrum_decomposition::
		   p_trace_strain_negative_p_temperature(strain_e, m_alpha);

      // partial stress negative partial temperature
      SymmetricTensor<2, dim> stress_negative_temperature
                            = m_lame_lambda
  	                    * trace_strain_negative_temperature
  		            * Physics::Elasticity::StandardTensors<dim>::I
  			    + 2.0 * m_lame_mu * strain_negative_temperature;

      // partial stress partial temperature
      m_p_stress_p_t = degradation * stress_positive_temperature
	             + stress_negative_temperature;
    }
  ...
  private:
    ...
    void assemble_system_one_cell_ut(
      ...
      ScratchData_ASM_UT                                   &scratch,
      PerTaskData_ASM_UT                                   &data) const;
    void assemble_rhs_ut();
    void assemble_rhs_one_cell_ut(
      const typename DoFHandler<dim>::active_cell_iterator &cell,
      ScratchData_RHS_UT &                           scratch,
      PerTaskData_RHS_UT &                           data) const;
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::get_error_update_ut(const BlockVector<double> &soln_update,
                                                  Errors_ut & error_update)
  {
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::read_material_data(const std::string &data_file,
				                 const unsigned int total_material_regions)
  {
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::read_time_data(const std::string &data_file,
				             std::vector<std::array<double, 4>> & time_table)
  {
    std::ifstream myfile (data_file);
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::setup_qph()
  {
  ...
  template <int dim>
  void
  SplitSolveUTandD<dim>::update_qph_incremental(const BlockVector<double> &solution_delta_ut)
  {
    m_timer.enter_subsection("Update QPH data");

    const BlockVector<double> solution_total_ut(get_total_solution_ut(solution_delta_ut));

    const UpdateFlags uf_UQPH(update_values | update_gradients);
    PerTaskData_UQPH  per_task_data_UQPH;
    ScratchData_UQPH  scratch_data_UQPH(m_fe_ut,
					m_fe_d,
					m_qf_cell,
					uf_UQPH,
					solution_total_ut,
    ...
    WorkStream::run(
	IteratorPair(IteratorTuple(m_dof_handler_ut.begin_active(),
  ...
  template <int dim>
  struct SplitSolveUTandD<dim>::PerTaskData_UQPH
  {
    void reset()
    {}
  };
  ...
  template <int dim>
  struct SplitSolveUTandD<dim>::ScratchData_UQPH
  {
  ...
  template <int dim>
  struct SplitSolveUTandD<dim>::PerTaskData_ASM_UT
  {
  ...
  template <int dim>
  struct SplitSolveUTandD<dim>::PerTaskData_ASM_D
  {
    FullMatrix<double>                   m_cell_matrix;
    Vector<double>                       m_cell_rhs;
    std::vector<types::global_dof_index> m_local_dof_indices;
  ...
  template <int dim>
  struct SplitSolveUTandD<dim>::PerTaskData_RHS_D
  {
    Vector<double>                       m_cell_rhs;
    std::vector<types::global_dof_index> m_local_dof_indices;
  ...
  template <int dim>
  struct SplitSolveUTandD<dim>::PerTaskData_RHS_UT
  {
    Vector<double>                       m_cell_rhs;
    std::vector<types::global_dof_index> m_local_dof_indices;
  ...
  template <int dim>
  struct SplitSolveUTandD<dim>::ScratchData_ASM_UT
  {
  ...
  template <int dim>
  struct SplitSolveUTandD<dim>::ScratchData_ASM_D
  {
    FEValues<dim>     m_fe_values;
  ...
  template <int dim>
  struct SplitSolveUTandD<dim>::ScratchData_RHS_UT
  {
    FEValues<dim>     m_fe_values;
    FEFaceValues<dim> m_fe_face_values;
  ...
  template <int dim>
  struct SplitSolveUTandD<dim>::ScratchData_RHS_D
  {
    FEValues<dim>     m_fe_values;
  ...
  SplitSolveUTandD<dim>::SplitSolveUTandD(const std::string &input_file)
    ...
    , m_dof_handler_ut(m_triangulation)
    ...
    , m_dofs_per_block_ut(m_n_blocks_ut)
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::make_grid()
  {
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::make_grid_case_1()
  {
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::make_grid_case_2()
  {
    for (unsigned int i = 0; i < 80; ++i)
      m_logfile << "*";
    m_logfile << std::endl;
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::make_grid_case_3()
  {
    for (unsigned int i = 0; i < 80; ++i)
      m_logfile << "*";
    m_logfile << std::endl;
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::make_grid_case_4()
  {
    for (unsigned int i = 0; i < 80; ++i)
      m_logfile << "*";
    m_logfile << std::endl;
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::make_grid_case_5()
  {
    for (unsigned int i = 0; i < 80; ++i)
      m_logfile << "*";
    m_logfile << std::endl;
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::make_grid_case_6()
  {
    for (unsigned int i = 0; i < 80; ++i)
      m_logfile << "*";
    m_logfile << std::endl;
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::setup_system()
  {
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::setup_system_d()
  {
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::setup_system_ut()
  {
    ...
    m_tangent_matrix_ut.clear();
    {
      ...
      DoFTools::make_sparsity_pattern(
        m_dof_handler_ut, coupling, dsp, m_constraints_ut, false);
						  ...
						  const unsigned int itr_stagger)
  {
    ...
    if (apply_dirichlet_bc)
      {
	m_constraints_ut.clear();
	DoFTools::make_hanging_node_constraints(m_dof_handler_ut,
						m_constraints_ut);
	...
	if (   m_parameters.m_scenario == 1
	    || m_parameters.m_scenario == 3)
	  {
	    ...
	    VectorTools::interpolate_boundary_values(m_dof_handler_ut,
						     boundary_id_bottom_surface,
						     Functions::ZeroFunction<dim>(m_n_components_ut),
						     m_constraints_ut,
	    ...
	    VectorTools::interpolate_boundary_values(m_dof_handler_ut,
						     ...
						     Functions::ConstantFunction<dim>(
						       delta_temperature, m_n_components_ut),
	    					     m_constraints_ut,
	    					     m_fe_ut.component_mask(temperature));
		...
		if (   (std::fabs(vertex_itr->vertex()[0] - 0.0) < 1.0e-9)
		  ...
		  {
		    node_xy = usr_utilities::get_vertex_dofs(vertex_itr, m_dof_handler_ut);
		  }
	      }
	    ...
	    */
	    ...
	    VectorTools::interpolate_boundary_values(m_dof_handler_ut,
						     boundary_id_top_surface,
						     Functions::ConstantFunction<dim>(
						       disp_magnitude*time_inc, m_n_components_ut),
						     m_constraints_ut,
						     m_fe_ut.component_mask(y_displacement));
...
//	      delta_temperature = -time_inc * 2.0e4; //cool down
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::assemble_system_d()
  {
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::assemble_system_ut()
  {
    ...
    ScratchData_ASM_UT scratch_data(m_fe_ut, m_qf_cell, uf_cell, m_qf_face, uf_face,
				    m_solution_previous_ut);

    auto worker =
      [this](const typename DoFHandler<dim>::active_cell_iterator &cell,
	     ScratchData_ASM_UT & scratch,
	     PerTaskData_ASM_UT & data)
      {
    ...
    auto copier = [this](const PerTaskData_ASM_UT &data)
      {
        this->m_constraints_ut.distribute_local_to_global(data.m_cell_matrix,
							  data.m_cell_rhs,
                                                          data.m_local_dof_indices,
						          m_tangent_matrix_ut,
							  m_system_rhs_ut);
    ...
    WorkStream::run(
      m_dof_handler_ut.active_cell_iterators(),
      worker,
      copier,
      scratch_data,
      per_task_data);
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::assemble_rhs_ut()
  {
    ...
    ScratchData_RHS_UT scratch_data(m_fe_ut, m_qf_cell, uf_cell, m_qf_face, uf_face,
				    m_solution_previous_ut);

    auto worker =
      [this](const typename DoFHandler<dim>::active_cell_iterator &cell,
	     ScratchData_RHS_UT & scratch,
	     PerTaskData_RHS_UT & data)
      {
    ...
    auto copier = [this](const PerTaskData_RHS_UT &data)
      {
        this->m_constraints_ut.distribute_local_to_global(data.m_cell_rhs,
                                                          data.m_local_dof_indices,
							  m_system_rhs_ut);
      };
      ...
      PerTaskData_RHS_UT & data) const
  {
    ...
    scratch.m_fe_values[m_t_fe].get_function_values(
      scratch.m_solution_ut_previous_step, scratch.m_temperature_previous_step_cell);
              ...
              for (const unsigned int i : scratch.m_fe_values.dof_indices())
                {
                  ...
                  if (i_group == m_u_dof)
                    {
    		      const unsigned int component_i = m_fe_ut.system_to_component_index(i).first;
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::assemble_rhs_d()
  {
      ...
      PerTaskData_ASM_UT & data) const
  {
	...
	// partial stress partial temperature
	const SymmetricTensor<2, dim> & p_stress_p_t = lqph[q_point]->get_p_stress_p_t();
	...
	const SymmetricTensor<2, dim> & old_strain = scratch.m_strain_previous_step_cell[q_point];
		...
		data.m_cell_rhs(i) += N_disp[i] * rhs_values[q_point] * JxW;
            ...
            for (const unsigned int j : scratch.m_fe_values.dof_indices())
              {
                const unsigned int j_group = m_fe_ut.system_to_base_index(j).first.first;
                ...
                if ((i_group == j_group) && (i_group == m_u_dof))
                  ...
                  {
                    data.m_cell_matrix(i, j) += symm_grad_N_disp[i]
				              * p_stress_p_t
					      * N_temperature[j]
					      * JxW;
                  }
                  ...
                  {
                    data.m_cell_matrix(i, j) += N_temperature[i]
					      * ut_coupling_tensor
					      * symm_grad_N_disp[j]
					      * JxW;
                  }
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::update_history_field_step()
  {
    for (const auto &cell : m_triangulation.active_cell_iterators())
      {
  ...
  template <int dim>
  unsigned int SplitSolveUTandD<dim>::displacement_thermal_step(unsigned int itr_stagger)
  {
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::phasefield_step()
  {
    make_constraints_d();
    assemble_system_d();
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::make_constraints_d()
  {
    m_constraints_d.clear();
    DoFTools::make_hanging_node_constraints(m_dof_handler_d, m_constraints_d);
    m_constraints_d.close();
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::
  solve_linear_system_ut(BlockVector<double> & newton_update_ut)
  {
    ...
    /*
	    ...
	    SparseILU_u.initialize(m_tangent_matrix_ut.block(m_u_dof, m_u_dof));
	    SparseILU<double> SparseILU_t;
	    SparseILU_t.initialize(m_tangent_matrix_ut.block(m_t_dof, m_t_dof));
	    usr_sparseILU_preconditioner preconditioner(SparseILU_u,
							SparseILU_t);
    */
	    GMRES_solver_ut.solve(m_tangent_matrix_ut,
				  newton_update_ut,
				  m_system_rhs_ut,
				  preconditioner);
	  }
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::solve_linear_system_d()
  {
  ...
  template <int dim>
  unsigned int SplitSolveUTandD<dim>::
  solve_nonlinear_newton_ut(BlockVector<double> & solution_delta_ut,
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::output_results() const
  {
    ...
    data_out.add_data_vector(m_dof_handler_ut,
	                     m_solution_ut,
                             solution_name_ut,
                             data_component_interpretation_ut);
  ...
  void SplitSolveUTandD<dim>::calculate_reaction_force(unsigned int face_ID)
  {
    ...
    std::vector<std::vector<Tensor<1, dim>>>
      Nx(m_qf_cell.size(), std::vector<Tensor<1, dim>>(m_dofs_per_cell_ut));
    std::vector<std::vector<Tensor<2, dim>>>
      grad_Nx(m_qf_cell.size(), std::vector<Tensor<2, dim>>(m_dofs_per_cell_ut));
    std::vector<std::vector<SymmetricTensor<2, dim>>>
      symm_grad_Nx(m_qf_cell.size(), std::vector<SymmetricTensor<2, dim>>(m_dofs_per_cell_ut));
    ...
    DoFTools::map_dof_to_boundary_indices(m_dof_handler_ut,
					  boundary_ids,
					  mapping);
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::write_history_data()
  {
  ...
  std::pair<double, double>
    SplitSolveUTandD<dim>::calculate_total_strain_energy_and_crack_energy_dissipation() const
  {
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::print_conv_header()
  {
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::print_parameter_information()
  {
    ...
    m_logfile << "Nonlinear solver type for the thermomechanical (ut) subproblem = "
	      << m_parameters.m_type_nonlinear_solver << std::endl;
    m_logfile << "Newton tolerance for ut-subproblem = " << m_parameters.m_Newton_ut_tol << std::endl;
	...
	if (cell_refine_flag)
	  {
	    ...
	    SolutionTransfer<dim, BlockVector<double>> solution_transfer_ut(m_dof_handler_ut);
	    solution_transfer_ut.prepare_for_coarsening_and_refinement(m_solution_previous_ut);
...
#  else
	    ...
	    solution_transfer_ut.interpolate(m_solution_previous_ut, tmp_solution_previous_ut);
#  endif
	...
	unsigned int adp_refine_iteration = 0;
		...
		if (   (iter_stagger > 1)
		    ...
		    && (d_inc_l2 < m_parameters.m_tol_d_incr)   )
		  {
		    ...
		    m_logfile << "\t\tAbsolute residual of thermomechanical (ut) equations: "
			      << ut_residual_l2 << std::endl;

		    m_logfile << "\t\tAbsolute residual of phasefield (d) equation: "
			      << d_residual_l2 << std::endl;

		    m_logfile << "\t\tAbsolute increment of displacement and temperature (ut): "
			      << ut_inc_l2 << std::endl;
