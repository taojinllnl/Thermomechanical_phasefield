 ...
 * crack problem. The entire problem is decoupled into two subproblems, the
 * damage (phase-field) problem and the thermomechanical problem (u and T):
 ...
 * 5. This code implements a hybrid staggered approach. The displacement and
 * temperature are updated together. The phase-field is updated separately. The
 * phase-field irreversibility is enforced through the history field Phi_0^+.
 * 6. Using TBB for stiffness assembly and Gauss point calculation.
 * 7. Using adaptive mesh refinement.
 * 8. The displacement and temperature are solved together using Newton method,
 * and the phase-field problem itself is linear.
 */
...
namespace PhaseField_uT_and_d
{
  ...
  // Incomplete LU preconditioner for GMRES solve of the ut system
  /*
    class usr_sparseILU_preconditioner : public Subscriptor
    {
    public:
      usr_sparseILU_preconditioner(const SparseILU<double> & ILU_factorization_u,
                                   const SparseILU<double> & ILU_factorization_t);

      void vmult(BlockVector<double> & dst,
                 const BlockVector<double> & src) const;

    private:
      const ObserverPointer<const SparseILU<double> > m_ILU_factorization_u;
      const ObserverPointer<const SparseILU<double> > m_ILU_factorization_t;
    };

    usr_sparseILU_preconditioner::usr_sparseILU_preconditioner(const
    SparseILU<double> & ILU_factorization_u, const SparseILU<double> &
    ILU_factorization_t) : m_ILU_factorization_u(& ILU_factorization_u) ,
    m_ILU_factorization_t(& ILU_factorization_t)
    {}
    ...
    const
    {
      ...
      (*m_ILU_factorization_u).vmult(dst.block(0), src.block(0));
      (*m_ILU_factorization_t).vmult(dst.block(1), src.block(1));
    }
  */
  ...
  namespace Parameters
  {
    struct Scenario
    {
      ...
      double m_gmres_ut_tol;
      double m_Newton_ut_tol;
    ...
    void Scenario::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Scenario");
      {
        ...
        prm.declare_entry("Nonlinear solver type", "Newton",
                          Patterns::Selection("Newton"),
                          "Type of solver used to solve the thermomechanical (ut) "
                          "nonlinear system");
        ...
        prm.declare_entry("GMRES ut tolerance", "1.0e-9", Patterns::Double(0.0),
                          "If Iterative is selected as linear solver, the "
                          "tolerance of GMRES for ut-subproblem");

        prm.declare_entry("Ut Newton tolerance", "1.0e-9", Patterns::Double(0.0),
                          "Tolerance of Newton iterations for ut-subproblem");

        prm.declare_entry(
            "Mesh refinement strategy", "pre-refine",
    ...
    void Scenario::parse_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Scenario");
      {
        ...
        m_gmres_ut_tol = prm.get_double("GMRES ut tolerance");
        m_Newton_ut_tol = prm.get_double("Ut Newton tolerance");
    ...
    struct NonlinearSolver
    {
      ...
      unsigned int m_max_iterations_ut;

      double m_tol_ut_residual;
      double m_tol_d_residual;

      double m_tol_ut_incr;
      double m_tol_d_incr;
    ...
    void NonlinearSolver::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Nonlinear solver");
      {
        ...
        prm.declare_entry(
            "Max iterations UT", "20", Patterns::Integer(0),
            "Number of Newton iterations allowed for the UT subproblem");

        prm.declare_entry("Tolerance ut residual", "1.0e-9", Patterns::Double(0.0),
                          "Thermomechanical residual tolerance");

        prm.declare_entry("Tolerance phasefield residual", "1.0e-9",
                          Patterns::Double(0.0), "Phasefield residual tolerance");

        prm.declare_entry("Tolerance ut increment", "1.0e-9", Patterns::Double(0.0),
                          "Thermomechanical increment tolerance");
    ...
    void NonlinearSolver::parse_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Nonlinear solver");
      {
        ...
        m_max_iterations_ut = prm.get_integer("Max iterations UT");

        m_tol_ut_residual = prm.get_double("Tolerance ut residual");
        m_tol_d_residual = prm.get_double("Tolerance phasefield residual");

        m_tol_ut_incr = prm.get_double("Tolerance ut increment");
  ...
  public:
    LinearIsotropicElasticityAdditiveSplit(
        ...
        : m_lame_lambda(lame_lambda), m_lame_mu(lame_mu),
          ...
          m_p_stress_p_t(SymmetricTensor<2, dim>())
    {
    ...
    const SymmetricTensor<2, dim> &get_p_stress_p_t() const
    {
      return m_p_stress_p_t;
    }
  ...
  private:
    ...
    SymmetricTensor<2, dim> m_p_stress_p_t;
  };
  ...
  void LinearIsotropicElasticityAdditiveSplit<dim>::update_material_data(
      ...
      const double temperature, const Tensor<1, dim> &grad_temperature,
      const bool degrade_conductivity_or_not)
  {
    ...
    usr_spectrum_decomposition::spectrum_decomposition<dim>(strain_e, eigenvalues,
                                                            eigenvectors);
    ...
    // partial strain positive partial temperature
    SymmetricTensor<2, dim> strain_positive_temperature =
        usr_spectrum_decomposition::p_strain_positive_p_temperature(
            eigenvalues, eigenvectors, m_alpha);

    // partial trace strain positive partial temperature
    const double trace_strain_positive_temperature =
        usr_spectrum_decomposition::p_trace_strain_positive_p_temperature(
            strain_e, m_alpha);

    // partial stress positive partial temperature
    SymmetricTensor<2, dim> stress_positive_temperature =
        my_lambda * trace_strain_positive_temperature *
            Physics::Elasticity::StandardTensors<dim>::I +
        2.0 * m_lame_mu * strain_positive_temperature;

    // partial strain negative partial temperature
    SymmetricTensor<2, dim> strain_negative_temperature =
        usr_spectrum_decomposition::p_strain_negative_p_temperature(
            eigenvalues, eigenvectors, m_alpha);

    // partial trace strain negative partial temperature
    const double trace_strain_negative_temperature =
        usr_spectrum_decomposition::p_trace_strain_negative_p_temperature(
            strain_e, m_alpha);

    // partial stress negative partial temperature
    SymmetricTensor<2, dim> stress_negative_temperature =
        my_lambda * trace_strain_negative_temperature *
            Physics::Elasticity::StandardTensors<dim>::I +
        2.0 * m_lame_mu * strain_negative_temperature;

    // partial stress partial temperature
    m_p_stress_p_t =
        degradation * stress_positive_temperature + stress_negative_temperature;
  }
  ...
  public:
    ...
    void setup_lqp(
        const double lame_lambda, const double lame_mu, const double length_scale,
        const double gc_0, const double viscosity, const double residual_k,
        const double heat_capacity, const double thermal_conductivity_0,
        const double thermal_expansion_coeff, const double reference_temperature,
        const double max_temperature, const double b_1, const double b_2,
    ...
    const SymmetricTensor<2, dim> &get_p_stress_p_t() const
    {
      return m_material->get_p_stress_p_t();
    }
  ...
  template <int dim> class SplitSolveUTandD
  {
  public:
    SplitSolveUTandD(const std::string &input_file);

    virtual ~SplitSolveUTandD() = default;
    void run();
  ...
  private:
    ...
    struct PerTaskData_ASM_UT;
    struct ScratchData_ASM_UT;

    struct PerTaskData_RHS_UT;
    struct ScratchData_RHS_UT;
    ...
    DoFHandler<dim> m_dof_handler_ut;
    DoFHandler<dim> m_dof_handler_d;

    FESystem<dim> m_fe_ut;
    FESystem<dim> m_fe_d;

    const unsigned int m_dofs_per_cell_ut;

    const FEValuesExtractors::Vector m_u_fe;
    const FEValuesExtractors::Scalar m_t_fe;

    static const unsigned int m_n_blocks_ut = 2;
    static const unsigned int m_n_components_ut = dim + 1;
    static const unsigned int m_first_u_component = 0;
    static const unsigned int m_t_component = dim;
    ...
    std::vector<types::global_dof_index> m_dofs_per_block_ut;
    ...
    AffineConstraints<double> m_constraints_ut;
    BlockSparsityPattern m_sparsity_pattern_ut;
    BlockSparseMatrix<double> m_tangent_matrix_ut;
    BlockVector<double> m_system_rhs_ut;
    BlockVector<double> m_solution_ut;
    BlockVector<double> m_solution_previous_ut;
    ...
    struct Errors_ut
    {
      Errors_ut() : m_norm(1.0), m_u(1.0), m_t(1.0) {}
      ...
      void normalize(const Errors_ut &rhs)
      {
        if (rhs.m_norm != 0.0)
          m_norm /= rhs.m_norm;
        if (rhs.m_u != 0.0)
      ...
      double m_norm, m_u, m_t;
    };

    Errors_ut m_error_residual_ut, m_error_update_ut;

    void get_error_residual_ut(Errors_ut &error_residual);
    void get_error_update_ut(const BlockVector<double> &soln_update,
                             Errors_ut &error_update);
    ...
    void setup_system();
    void setup_system_ut();
    void setup_system_d();

    void setup_temperature_initial_conditions();

    unsigned int displacement_thermal_step(unsigned int itr_stagger);
    void make_constraints_ut(const unsigned int it_nr,
                             const unsigned int itr_stagger);
    void assemble_system_ut();
    void assemble_system_one_cell_ut(
        const typename DoFHandler<dim>::active_cell_iterator &cell,
        ScratchData_ASM_UT &scratch, PerTaskData_ASM_UT &data) const;
    void assemble_rhs_ut();
    void assemble_rhs_one_cell_ut(
        const typename DoFHandler<dim>::active_cell_iterator &cell,
        ScratchData_RHS_UT &scratch, PerTaskData_RHS_UT &data) const;
    unsigned int solve_nonlinear_newton_ut(BlockVector<double> &solution_delta_ut,
                                           unsigned int itr_stagger);
    void solve_linear_system_ut(BlockVector<double> &newton_update_ut);
    ...
    void assemble_system_one_cell_d(
        const typename DoFHandler<dim>::active_cell_iterator &cell,
        ScratchData_ASM_D &scratch, PerTaskData_ASM_D &data) const;
    void assemble_rhs_d();
    ...
    void update_qph_incremental(const BlockVector<double> &solution_delta_ut);
    ...
    BlockVector<double>
    get_total_solution_ut(const BlockVector<double> &solution_delta_ut) const;
  ...
  }; // class SplitSolveUTandD

  template <int dim>
  void SplitSolveUTandD<dim>::get_error_residual_ut(Errors_ut &error_residual)
  {
    BlockVector<double> error_res(m_dofs_per_block_ut);

    for (unsigned int i = 0; i < m_dof_handler_ut.n_dofs(); ++i)
      if (!m_constraints_ut.is_constrained(i))
        error_res(i) = m_system_rhs_ut(i);
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::get_error_update_ut(
      const BlockVector<double> &soln_update, Errors_ut &error_update)
  {
    BlockVector<double> error_ud(m_dofs_per_block_ut);
    for (unsigned int i = 0; i < m_dof_handler_ut.n_dofs(); ++i)
      if (!m_constraints_ut.is_constrained(i))
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::read_material_data(
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::read_time_data(
  ...
  template <int dim> void SplitSolveUTandD<dim>::setup_qph()
  {
  ...
  template <int dim>
  BlockVector<double> SplitSolveUTandD<dim>::get_total_solution_ut(
      const BlockVector<double> &solution_delta_ut) const
  {
    BlockVector<double> solution_total_ut(m_solution_ut);
    solution_total_ut += solution_delta_ut;
    return solution_total_ut;
  }

  template <int dim>
  void SplitSolveUTandD<dim>::update_qph_incremental(
      const BlockVector<double> &solution_delta_ut)
  {
    m_timer.enter_subsection("Update QPH data");

    const BlockVector<double> solution_total_ut(
        get_total_solution_ut(solution_delta_ut));

    const UpdateFlags uf_UQPH(update_values | update_gradients);
    PerTaskData_UQPH per_task_data_UQPH;
    ScratchData_UQPH scratch_data_UQPH(
        m_fe_ut, m_fe_d, m_qf_cell, uf_UQPH, solution_total_ut, m_solution_d,
        m_solution_previous_d, m_time.get_delta_t(),
    ...
    WorkStream::run(IteratorPair(IteratorTuple(m_dof_handler_ut.begin_active(),
                                               m_dof_handler_d.begin_active())),
                    IteratorPair(IteratorTuple(m_dof_handler_ut.end(),
                                               m_dof_handler_d.end())),
  ...
  template <int dim> struct SplitSolveUTandD<dim>::PerTaskData_UQPH
  {
    void reset() {}
  };

  template <int dim> struct SplitSolveUTandD<dim>::ScratchData_UQPH
  {
    const BlockVector<double> &m_solution_ut_UQPH;
    ...
    ScratchData_UQPH(const FiniteElement<dim> &fe_cell_ut,
                     ...
                     const bool degrade_conductivity_or_not)
        : m_solution_ut_UQPH(solution_total_ut),
          ...
          m_fe_values_ut(fe_cell_ut, qf_cell, uf_cell),
          m_fe_values_d(fe_cell_d, qf_cell, uf_cell), m_delta_time(delta_time),
          m_degrade_conductivity_or_not(degrade_conductivity_or_not)
    {
    }

    ScratchData_UQPH(const ScratchData_UQPH &rhs)
        : m_solution_ut_UQPH(rhs.m_solution_ut_UQPH),
          ...
          m_fe_values_ut(rhs.m_fe_values_ut.get_fe(),
                         rhs.m_fe_values_ut.get_quadrature(),
                         rhs.m_fe_values_ut.get_update_flags()),
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::update_qph_incremental_one_cell(
      const IteratorPair &synchronous_iterators, ScratchData_UQPH &scratch,
      PerTaskData_UQPH & /*data*/)
  {
    scratch.reset();

    scratch.m_fe_values_ut.reinit(std::get<0>(*synchronous_iterators));
    scratch.m_fe_values_d.reinit(std::get<1>(*synchronous_iterators));

    const std::vector<std::shared_ptr<PointHistory<dim>>> lqph =
        m_quadrature_point_history.get_data(std::get<0>(*synchronous_iterators));
    Assert(lqph.size() == m_n_q_points, ExcInternalError());

    scratch.m_fe_values_ut[m_u_fe].get_function_symmetric_gradients(
        scratch.m_solution_ut_UQPH, scratch.m_solution_symm_grads_u_cell);
    scratch.m_fe_values_ut[m_t_fe].get_function_values(
        scratch.m_solution_ut_UQPH, scratch.m_solution_values_temperature_cell);
    scratch.m_fe_values_ut[m_t_fe].get_function_gradients(
        scratch.m_solution_ut_UQPH, scratch.m_solution_grad_temperature_cell);
    ...
    scratch.m_fe_values_d.get_function_values(
        scratch.m_solution_previous_d_UQPH,
        scratch.m_phasefield_previous_step_cell);

    for (const unsigned int q_point :
         scratch.m_fe_values_ut.quadrature_point_indices())
  ...
  template <int dim> struct SplitSolveUTandD<dim>::PerTaskData_ASM_UT
  {
    FullMatrix<double> m_cell_matrix;
    Vector<double> m_cell_rhs;
    std::vector<types::global_dof_index> m_local_dof_indices;

    PerTaskData_ASM_UT(const unsigned int dofs_per_cell)
  ...
  template <int dim> struct SplitSolveUTandD<dim>::PerTaskData_ASM_D
  {
    FullMatrix<double> m_cell_matrix;
    Vector<double> m_cell_rhs;
    std::vector<types::global_dof_index> m_local_dof_indices;
  ...
  template <int dim> struct SplitSolveUTandD<dim>::PerTaskData_RHS_D
  {
    Vector<double> m_cell_rhs;
    std::vector<types::global_dof_index> m_local_dof_indices;
  ...
  template <int dim> struct SplitSolveUTandD<dim>::PerTaskData_RHS_UT
  {
    Vector<double> m_cell_rhs;
    std::vector<types::global_dof_index> m_local_dof_indices;

    PerTaskData_RHS_UT(const unsigned int dofs_per_cell)
        : m_cell_rhs(dofs_per_cell), m_local_dof_indices(dofs_per_cell)
    {
    }
  ...
  template <int dim> struct SplitSolveUTandD<dim>::ScratchData_ASM_UT
  {
    ...
    ScratchData_ASM_UT(const FiniteElement<dim> &fe_cell,
        ...
        : m_fe_values(fe_cell, qf_cell, uf_cell),
          ...
          m_solution_ut_previous_step(solution_old_ut),
          m_strain_previous_step_cell(qf_cell.size()),
          m_temperature_previous_step_cell(qf_cell.size())
    {
    }

    ScratchData_ASM_UT(const ScratchData_ASM_UT &rhs)
        : m_fe_values(rhs.m_fe_values.get_fe(), rhs.m_fe_values.get_quadrature(),
          ...
          m_solution_ut_previous_step(rhs.m_solution_ut_previous_step),
  ...
  template <int dim> struct SplitSolveUTandD<dim>::ScratchData_ASM_D
  {
    FEValues<dim> m_fe_values;
  ...
  template <int dim> struct SplitSolveUTandD<dim>::ScratchData_RHS_UT
  {
    FEValues<dim> m_fe_values;
    FEFaceValues<dim> m_fe_face_values;
    ...
    ScratchData_RHS_UT(const FiniteElement<dim> &fe_cell,
                       const QGauss<dim> &qf_cell, const UpdateFlags uf_cell,
                       const QGauss<dim - 1> &qf_face, const UpdateFlags uf_face,
                       const BlockVector<double> &solution_old_ut)
        : m_fe_values(fe_cell, qf_cell, uf_cell),
    ...
    ScratchData_RHS_UT(const ScratchData_RHS_UT &rhs)
        : m_fe_values(rhs.m_fe_values.get_fe(), rhs.m_fe_values.get_quadrature(),
                      rhs.m_fe_values.get_update_flags()),
          m_fe_face_values(rhs.m_fe_face_values.get_fe(),
                           rhs.m_fe_face_values.get_quadrature(),
  ...
  template <int dim> struct SplitSolveUTandD<dim>::ScratchData_RHS_D
  {
    FEValues<dim> m_fe_values;
  ...
  template <int dim>
  SplitSolveUTandD<dim>::SplitSolveUTandD(const std::string &input_file)
      : m_parameters(input_file),
        ...
        m_dof_handler_ut(m_triangulation), m_dof_handler_d(m_triangulation),
        m_fe_ut(FE_Q<dim>(m_parameters.m_poly_degree),
                dim, // displacement
        ...
        m_fe_d(FE_Q<dim>(m_parameters.m_poly_degree),
               1) // phase-field
        ,
        m_dofs_per_cell_ut(m_fe_ut.n_dofs_per_cell()),
        m_u_fe(m_first_u_component), m_t_fe(m_t_component),
        m_dofs_per_block_ut(m_n_blocks_ut), m_qf_cell(m_parameters.m_quad_order),
        m_qf_face(m_parameters.m_quad_order), m_n_q_points(m_qf_cell.size()),
        m_vol_reference(0.0)
  {
  }

  template <int dim> void SplitSolveUTandD<dim>::make_grid()
  {
  ...
  template <int dim> void SplitSolveUTandD<dim>::make_grid_case_1()
  {
  ...
  template <int dim> void SplitSolveUTandD<dim>::make_grid_case_2()
  {
    for (unsigned int i = 0; i < 80; ++i)
      m_logfile << "*";
    m_logfile << std::endl;
  ...
  template <int dim> void SplitSolveUTandD<dim>::make_grid_case_3()
  {
    for (unsigned int i = 0; i < 80; ++i)
      m_logfile << "*";
    m_logfile << std::endl;
  ...
  template <int dim> void SplitSolveUTandD<dim>::make_grid_case_4()
  {
    for (unsigned int i = 0; i < 80; ++i)
      m_logfile << "*";
    m_logfile << std::endl;
  ...
  template <int dim> void SplitSolveUTandD<dim>::make_grid_case_5()
  {
    for (unsigned int i = 0; i < 80; ++i)
      m_logfile << "*";
    m_logfile << std::endl;
  ...
  template <int dim> void SplitSolveUTandD<dim>::make_grid_case_6()
  {
    for (unsigned int i = 0; i < 80; ++i)
      m_logfile << "*";
    m_logfile << std::endl;
  ...
  template <int dim> void SplitSolveUTandD<dim>::setup_system()
  {
    ...
    m_logfile << "\t\tTriangulation:"
              ...
              << m_dof_handler_ut.n_dofs() + m_dof_handler_d.n_dofs()
              << "\n\t\t\t Number of degrees of freedom (disp): "
              << m_dofs_per_block_ut[m_u_dof]
              << "\n\t\t\t Number of degrees of freedom (temperature): "
              << m_dofs_per_block_ut[m_t_dof]
              << "\n\t\t\t Number of degrees of freedom (phasefield): "
              << m_dof_handler_d.n_dofs() << std::endl;
  ...
  template <int dim> void SplitSolveUTandD<dim>::setup_system_d()
  {
    ...
    DynamicSparsityPattern dsp(m_dof_handler_d.n_dofs(),
                               m_dof_handler_d.n_dofs());
    DoFTools::make_sparsity_pattern(m_dof_handler_d, dsp, m_constraints_d,
  ...
  template <int dim> void SplitSolveUTandD<dim>::setup_system_ut()
  {
    std::vector<unsigned int> block_component(m_n_components_ut,
                                              m_u_dof); // displacement
    block_component[m_t_component] = m_t_dof;           // temperature

    m_dof_handler_ut.distribute_dofs(m_fe_ut);
    DoFRenumbering::Cuthill_McKee(m_dof_handler_ut);
    DoFRenumbering::component_wise(m_dof_handler_ut, block_component);

    m_constraints_ut.clear();
    DoFTools::make_hanging_node_constraints(m_dof_handler_ut, m_constraints_ut);
    m_constraints_ut.close();

    m_dofs_per_block_ut =
        DoFTools::count_dofs_per_fe_block(m_dof_handler_ut, block_component);

    m_tangent_matrix_ut.clear();
    {
      BlockDynamicSparsityPattern dsp(m_dofs_per_block_ut, m_dofs_per_block_ut);

      Table<2, DoFTools::Coupling> coupling(m_n_components_ut, m_n_components_ut);
      for (unsigned int ii = 0; ii < m_n_components_ut; ++ii)
        for (unsigned int jj = 0; jj < m_n_components_ut; ++jj)
          coupling[ii][jj] = DoFTools::always;

      DoFTools::make_sparsity_pattern(m_dof_handler_ut, coupling, dsp,
                                      m_constraints_ut, false);
      m_sparsity_pattern_ut.copy_from(dsp);
    }

    m_tangent_matrix_ut.reinit(m_sparsity_pattern_ut);

    m_system_rhs_ut.reinit(m_dofs_per_block_ut);
    m_solution_ut.reinit(m_dofs_per_block_ut);
  }

  template <int dim>
  void SplitSolveUTandD<dim>::setup_temperature_initial_conditions()
  {
    if (m_parameters.m_scenario == 3 || m_parameters.m_scenario == 4)
    {
      for (unsigned int i = 0; i < m_dofs_per_block_ut[m_t_dof]; ++i)
      {
        m_solution_ut.block(m_t_dof)(i) = m_parameters.m_ref_temperature;
      }
    }
    else if (m_parameters.m_scenario == 5)
    {
      ...
      ComponentMask temperature_mask = m_fe_ut.component_mask(m_t_fe);
      support_points_T = DoFTools::map_dofs_to_support_points(
          MappingQ1<dim>(), m_dof_handler_ut, temperature_mask);
      ...
      for (auto const &item : support_points_T)
      {
        ...
        {
          m_solution_ut(item.first) = cool_down_temperature;
        }
      }
    }
    else if (m_parameters.m_scenario == 6)
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::make_constraints_ut(const unsigned int it_nr,
                                                  const unsigned int itr_stagger)
  {
    ...
    if (apply_dirichlet_bc)
    {
      ...
      if (m_parameters.m_scenario == 1 || m_parameters.m_scenario == 3)
      {
        ...
        VectorTools::interpolate_boundary_values(
            m_dof_handler_ut, boundary_id_bottom_surface,
            Functions::ZeroFunction<dim>(m_n_components_ut), m_constraints_ut,
            m_fe_ut.component_mask(y_displacement));

        // temperature B.C. at the bottom surface
        VectorTools::interpolate_boundary_values(
            m_dof_handler_ut, boundary_id_bottom_surface,
            Functions::ConstantFunction<dim>(delta_temperature,
                                             m_n_components_ut),
            m_constraints_ut, m_fe_ut.component_mask(temperature));

        typename Triangulation<dim>::active_vertex_iterator vertex_itr;
        vertex_itr = m_triangulation.begin_active_vertex();
        std::vector<types::global_dof_index> node_xy(m_fe_ut.dofs_per_vertex);
        ...
        for (; vertex_itr != m_triangulation.end_vertex(); ++vertex_itr)
        {
          ...
          {
            node_xy =
                usr_utilities::get_vertex_dofs(vertex_itr, m_dof_handler_ut);
          }
        }
        m_constraints_ut.add_line(node_xy[0]);
        m_constraints_ut.set_inhomogeneity(node_xy[0], 0.0);

        m_constraints_ut.add_line(node_xy[1]);
        m_constraints_ut.set_inhomogeneity(node_xy[1], 0.0);
        ...
        */
        ...
        VectorTools::interpolate_boundary_values(
            m_dof_handler_ut, boundary_id_top_surface,
            Functions::ConstantFunction<dim>(disp_magnitude * time_inc,
                                             m_n_components_ut),
            m_constraints_ut, m_fe_ut.component_mask(y_displacement));
      ...
      else if (m_parameters.m_scenario == 2 || m_parameters.m_scenario == 4)
      {
        ...
        VectorTools::interpolate_boundary_values(
            m_dof_handler_ut, boundary_id_bottom_surface,
            Functions::ZeroFunction<dim>(m_n_components_ut), m_constraints_ut,
            m_fe_ut.component_mask(displacements));
        ...
        VectorTools::interpolate_boundary_values(
            ...
            Functions::ConstantFunction<dim>(disp_magnitude * time_inc,
                                             m_n_components_ut),
            m_constraints_ut, m_fe_ut.component_mask(x_displacement));

        const int boundary_id_side_surfaces = 2;
        VectorTools::interpolate_boundary_values(
            m_dof_handler_ut, boundary_id_side_surfaces,
            Functions::ZeroFunction<dim>(m_n_components_ut), m_constraints_ut,
            m_fe_ut.component_mask(y_displacement));
      ...
      else if (m_parameters.m_scenario == 5)
      {
        ...
        VectorTools::interpolate_boundary_values(
            m_dof_handler_ut, boundary_id_mid_surface_x,
            Functions::ZeroFunction<dim>(m_n_components_ut), m_constraints_ut,
            m_fe_ut.component_mask(x_displacement));

        const int boundary_id_mid_surface_y = 3;
        VectorTools::interpolate_boundary_values(
            m_dof_handler_ut, boundary_id_mid_surface_y,
            Functions::ZeroFunction<dim>(m_n_components_ut), m_constraints_ut,
            m_fe_ut.component_mask(y_displacement));
        ...
        VectorTools::interpolate_boundary_values(
            m_dof_handler_ut, boundary_id_left_surface,
            Functions::ConstantFunction<dim>(delta_temperature,
                                             m_n_components_ut),
            m_constraints_ut, m_fe_ut.component_mask(temperature));
    ...
    {
      if (m_constraints_ut.has_inhomogeneities())
      {
        AffineConstraints<double> homogeneous_constraints(m_constraints_ut);
        for (unsigned int dof = 0; dof != m_dof_handler_ut.n_dofs(); ++dof)
          if (homogeneous_constraints.is_inhomogeneously_constrained(dof))
            homogeneous_constraints.set_inhomogeneity(dof, 0.0);
        m_constraints_ut.clear();
        m_constraints_ut.copy_from(homogeneous_constraints);
      }
    }
    m_constraints_ut.close();
  }

  template <int dim> void SplitSolveUTandD<dim>::assemble_system_d()
  {
  ...
  template <int dim> void SplitSolveUTandD<dim>::assemble_system_ut()
  {
    m_timer.enter_subsection("Assemble ut system");

    m_tangent_matrix_ut = 0.0;
    m_system_rhs_ut = 0.0;
    ...
    PerTaskData_ASM_UT per_task_data(m_fe_ut.n_dofs_per_cell());
    ScratchData_ASM_UT scratch_data(m_fe_ut, m_qf_cell, uf_cell, m_qf_face,
                                    uf_face, m_solution_previous_ut);

    auto worker =
        [this](const typename DoFHandler<dim>::active_cell_iterator &cell,
               ScratchData_ASM_UT &scratch, PerTaskData_ASM_UT &data) {
          this->assemble_system_one_cell_ut(cell, scratch, data);
        };

    auto copier = [this](const PerTaskData_ASM_UT &data) {
      this->m_constraints_ut.distribute_local_to_global(
          data.m_cell_matrix, data.m_cell_rhs, data.m_local_dof_indices,
          m_tangent_matrix_ut, m_system_rhs_ut);
    };

    WorkStream::run(m_dof_handler_ut.active_cell_iterators(), worker, copier,
                    scratch_data, per_task_data);

    m_timer.leave_subsection();
  }

  template <int dim> void SplitSolveUTandD<dim>::assemble_rhs_ut()
  {
    m_timer.enter_subsection("Assemble ut RHS");
    ...
    PerTaskData_RHS_UT per_task_data(m_fe_ut.n_dofs_per_cell());
    ScratchData_RHS_UT scratch_data(m_fe_ut, m_qf_cell, uf_cell, m_qf_face,
                                    uf_face, m_solution_previous_ut);

    auto worker =
        [this](const typename DoFHandler<dim>::active_cell_iterator &cell,
               ScratchData_RHS_UT &scratch, PerTaskData_RHS_UT &data) {
          this->assemble_rhs_one_cell_ut(cell, scratch, data);
        };

    auto copier = [this](const PerTaskData_RHS_UT &data) {
      this->m_constraints_ut.distribute_local_to_global(
          data.m_cell_rhs, data.m_local_dof_indices, m_system_rhs_ut);
    };
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::assemble_rhs_one_cell_ut(
      const typename DoFHandler<dim>::active_cell_iterator &cell,
      ScratchData_RHS_UT &scratch, PerTaskData_RHS_UT &data) const
  {
    data.reset();
    ...
    scratch.m_fe_values[m_u_fe].get_function_symmetric_gradients(
        scratch.m_solution_ut_previous_step, scratch.m_strain_previous_step_cell);

    scratch.m_fe_values[m_t_fe].get_function_values(
        scratch.m_solution_ut_previous_step,
         ...
         scratch.m_fe_values.quadrature_point_indices())
    {
      ...
      {
        const unsigned int k_group = m_fe_ut.system_to_base_index(k).first.first;
         ...
         scratch.m_fe_values.quadrature_point_indices())
    {
      ...
      {
        const unsigned int i_group = m_fe_ut.system_to_base_index(i).first.first;
             ...
             scratch.m_fe_face_values.quadrature_point_indices())
        {
          ...
          for (const unsigned int i : scratch.m_fe_values.dof_indices())
          {
            const unsigned int i_group =
                m_fe_ut.system_to_base_index(i).first.first;

            if (i_group == m_u_dof)
            {
              const unsigned int component_i =
                  m_fe_ut.system_to_component_index(i).first;
              const double Ni =
  ...
  template <int dim> void SplitSolveUTandD<dim>::assemble_rhs_d()
  {
    ...
    ScratchData_RHS_D scratch_data(m_fe_d, m_qf_cell, uf_cell,
                                   m_solution_previous_d);

    auto worker =
        [this](const typename DoFHandler<dim>::active_cell_iterator &cell,
               ScratchData_RHS_D &scratch, PerTaskData_RHS_D &data) {
          this->assemble_rhs_one_cell_d(cell, scratch, data);
        };

    auto copier = [this](const PerTaskData_RHS_D &data) {
      this->m_constraints_d.distribute_local_to_global(
          data.m_cell_rhs, data.m_local_dof_indices, m_system_rhs_d);
    };
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::assemble_system_one_cell_ut(
      const typename DoFHandler<dim>::active_cell_iterator &cell,
      ScratchData_ASM_UT &scratch, PerTaskData_ASM_UT &data) const
  {
    data.reset();
         ...
         scratch.m_fe_values.quadrature_point_indices())
    {
      ...
      // partial stress partial temperature
      const SymmetricTensor<2, dim> &p_stress_p_t =
          lqph[q_point]->get_p_stress_p_t();
      ...
      for (const unsigned int i : scratch.m_fe_values.dof_indices())
      {
        ...
        for (const unsigned int j : scratch.m_fe_values.dof_indices())
        {
          const unsigned int j_group =
              m_fe_ut.system_to_base_index(j).first.first;
          ...
          {
            data.m_cell_matrix(i, j) +=
                symm_grad_N_disp[i] * p_stress_p_t * N_temperature[j] * JxW;
          }
          else if ((i_group == m_t_dof) && (j_group == m_u_dof))
          {
            data.m_cell_matrix(i, j) +=
                N_temperature[i] * ut_coupling_tensor * symm_grad_N_disp[j] * JxW;
          }
          else
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::assemble_system_one_cell_d(
      const typename DoFHandler<dim>::active_cell_iterator &cell,
      ScratchData_ASM_D &scratch, PerTaskData_ASM_D &data) const
  {
    data.reset();
         ...
         scratch.m_fe_values.quadrature_point_indices())
    {
      for (const unsigned int k : scratch.m_fe_values.dof_indices())
      {
        scratch.m_Nx[q_point][k] = scratch.m_fe_values.shape_value(k, q_point);
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::assemble_rhs_one_cell_d(
      const typename DoFHandler<dim>::active_cell_iterator &cell,
      ScratchData_RHS_D &scratch, PerTaskData_RHS_D &data) const
  {
    data.reset();
  ...
  template <int dim> void SplitSolveUTandD<dim>::update_history_field_step()
  {
    for (const auto &cell : m_triangulation.active_cell_iterators())
    {
  ...
  template <int dim>
  unsigned int
  SplitSolveUTandD<dim>::displacement_thermal_step(unsigned int itr_stagger)
  {
    BlockVector<double> solution_delta_ut(m_dofs_per_block_ut);
    solution_delta_ut = 0.0;
    unsigned int newton_itrs_required =
        solve_nonlinear_newton_ut(solution_delta_ut, itr_stagger);
    m_solution_ut += solution_delta_ut;
    return newton_itrs_required;
  }

  template <int dim> void SplitSolveUTandD<dim>::phasefield_step()
  {
    make_constraints_d();
    assemble_system_d();
  ...
  template <int dim> void SplitSolveUTandD<dim>::make_constraints_d()
  {
    m_constraints_d.clear();
    DoFTools::make_hanging_node_constraints(m_dof_handler_d, m_constraints_d);
    m_constraints_d.close();
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::solve_linear_system_ut(
      BlockVector<double> &newton_update_ut)
  {
    m_timer.enter_subsection("Solve ut linear system");

    if (m_parameters.m_type_linear_solver == "Direct")
    {
      SparseDirectUMFPACK A_direct;
      A_direct.initialize(m_tangent_matrix_ut);
      A_direct.vmult(newton_update_ut, m_system_rhs_ut);
    }
    else if (m_parameters.m_type_linear_solver == "Iterative")
    {
      try
      {
        SolverControl solver_control_ut(1e4, m_parameters.m_gmres_ut_tol);
        ...
        preconditioner.initialize(m_tangent_matrix_ut, 1.0);
        /*
                SparseILU<double> SparseILU_u;
                SparseILU_u.initialize(m_tangent_matrix_ut.block(m_u_dof,
           m_u_dof)); SparseILU<double> SparseILU_t;
                SparseILU_t.initialize(m_tangent_matrix_ut.block(m_t_dof,
           m_t_dof)); usr_sparseILU_preconditioner preconditioner(SparseILU_u,
                                                            SparseILU_t);
        */
        GMRES_solver_ut.solve(m_tangent_matrix_ut, newton_update_ut,
                              m_system_rhs_ut, preconditioner);
      }
      catch (dealii::SolverControl::NoConvergence &exc)
      {
        ...
        SparseDirectUMFPACK A_direct;
        A_direct.initialize(m_tangent_matrix_ut);
        A_direct.vmult(newton_update_ut,
                       m_system_rhs_ut);
        */
        ;
      }
    ...
    else
    {
      AssertThrow(false, ExcMessage("Selected linear solver not implemented "
                                    "for the thermomechanical subproblem!"));
    }

    m_constraints_ut.distribute(newton_update_ut);

    m_timer.leave_subsection();
  }

  template <int dim> void SplitSolveUTandD<dim>::solve_linear_system_d()
  {
  ...
  template <int dim>
  unsigned int SplitSolveUTandD<dim>::solve_nonlinear_newton_ut(
      BlockVector<double> &solution_delta_ut, unsigned int iter_stagger)
  {
    BlockVector<double> newton_update_ut(m_dofs_per_block_ut);
    newton_update_ut = 0.0;

    m_error_residual_ut.reset();
    m_error_update_ut.reset();

    unsigned int newton_iteration = 0;
    for (; newton_iteration <= m_parameters.m_max_iterations_ut;
         ++newton_iteration)
    {
      ...
      )
      {
        if (m_parameters.m_output_iteration_history)
          m_logfile << "  \t" << newton_iteration << "\t" << std::setprecision(3)
                    ...
                    << m_error_residual_ut.m_norm << "  "
                    << m_error_update_ut.m_norm << std::flush;
        break;
      }

      solve_linear_system_ut(newton_update_ut);
      get_error_update_ut(newton_update_ut, m_error_update_ut);
      solution_delta_ut += newton_update_ut;

      update_qph_incremental(solution_delta_ut);
    }

    // AssertThrow(newton_iteration < m_parameters.m_max_iterations_ut,
    //            ExcMessage("No convergence in nonlinear solver for
    //            UT-subproblem!"));

    return newton_iteration;
  }

  template <int dim> void SplitSolveUTandD<dim>::output_results() const
  {
    ...
    std::vector<DataComponentInterpretation::DataComponentInterpretation>
        data_component_interpretation_ut(
            dim, DataComponentInterpretation::component_is_part_of_vector);

    data_component_interpretation_ut.push_back(
        DataComponentInterpretation::component_is_scalar);

    std::vector<std::string> solution_name_ut(dim, "displacement");
    solution_name_ut.emplace_back("temperature");

    data_out.add_data_vector(m_dof_handler_ut, m_solution_ut, solution_name_ut,
                             data_component_interpretation_ut);

    data_out.add_data_vector(m_dof_handler_d, m_solution_d, "phasefield");
    ...
    for (unsigned int i = 0; i < dim; ++i)
      for (unsigned int j = i; j < dim; ++j)
      {
        ...
        data_out.add_data_vector(dof_handler_L2, stress_field_L2, stress_name,
                                 data_component_interpretation_L2);
      }
    ...
    for (unsigned int i = 0; i < dim; ++i)
    {
      ...
      if (i == 0)
        ...
        AssertThrow(false, ExcMessage("Heat flux output is wrong!"));
    }
    ...
    data_out.add_data_vector(dof_handler_L2_flux, heat_flux_field_L2,
                             solution_name_flux,
                             data_component_interpretation_flux_L2);
  ...
  template <int dim>
  void SplitSolveUTandD<dim>::calculate_reaction_force(unsigned int face_ID)
  {
    m_timer.enter_subsection("Calculate reaction force");

    BlockVector<double> system_rhs;
    system_rhs.reinit(m_dofs_per_block_ut);

    Vector<double> cell_rhs(m_dofs_per_cell_ut);
    std::vector<types::global_dof_index> local_dof_indices(m_dofs_per_cell_ut);
    ...
    FEValues<dim> fe_values(m_fe_ut, m_qf_cell, uf_cell);
    FEFaceValues<dim> fe_face_values(m_fe_ut, m_qf_face, uf_face);

    // shape function values for displacement field
    std::vector<std::vector<Tensor<1, dim>>> Nx(
        m_qf_cell.size(), std::vector<Tensor<1, dim>>(m_dofs_per_cell_ut));
    std::vector<std::vector<Tensor<2, dim>>> grad_Nx(
        m_qf_cell.size(), std::vector<Tensor<2, dim>>(m_dofs_per_cell_ut));
    std::vector<std::vector<SymmetricTensor<2, dim>>> symm_grad_Nx(
        m_qf_cell.size(),
        std::vector<SymmetricTensor<2, dim>>(m_dofs_per_cell_ut));

    for (const auto &cell : m_dof_handler_ut.active_cell_iterators())
    {
      ...
      for (const unsigned int q_point : fe_values.quadrature_point_indices())
      {
        for (const unsigned int k : fe_values.dof_indices())
        {
          const unsigned int k_group =
              m_fe_ut.system_to_base_index(k).first.first;
    ...
    DoFTools::map_dof_to_boundary_indices(m_dof_handler_ut, boundary_ids,
                                          mapping);

    std::vector<double> reaction_force(dim, 0.0);

    for (unsigned int i = 0; i < m_dofs_per_block_ut[m_u_dof]; ++i)
    {
  ...
  template <int dim> void SplitSolveUTandD<dim>::write_history_data()
  {
  ...
  template <int dim>
  double SplitSolveUTandD<dim>::calculate_energy_functional() const
  {
    double energy_functional = 0.0;

    FEValues<dim> fe_values(m_fe_ut, m_qf_cell, update_JxW_values);
  ...
  template <int dim>
  std::pair<double, double> SplitSolveUTandD<
  ...
  template <int dim> void SplitSolveUTandD<dim>::print_conv_header()
  {
  ...
  template <int dim> void SplitSolveUTandD<dim>::print_parameter_information()
  {
    ...
    m_logfile
        << "Nonlinear solver type for the thermomechanical (ut) subproblem = "
        << m_parameters.m_type_nonlinear_solver << std::endl;
    m_logfile << "Newton tolerance for ut-subproblem = "
              << m_parameters.m_Newton_ut_tol << std::endl;
    m_logfile << "Linear solver type = " << m_parameters.m_type_linear_solver
              << std::endl;

    if (m_parameters.m_type_linear_solver == "Iterative")
    {
      m_logfile << "\tGMRES tolerance for ut-subproblem linear solver = "
                << m_parameters.m_gmres_ut_tol << std::endl;
    }
  ...
  template <int dim>
  bool SplitSolveUTandD<dim>::local_refine_and_solution_transfer()
  {
    ...
    while (cell_refine_flag)
    {
      ...
      if (cell_refine_flag)
      {
        ...
        SolutionTransfer<dim, BlockVector<double>> solution_transfer_ut(
            m_dof_handler_ut);
        solution_transfer_ut.prepare_for_coarsening_and_refinement(
            m_solution_previous_ut);
        ...
        tmp_solution_previous_ut.reinit(m_dofs_per_block_ut);
        ...
        solution_transfer_ut.interpolate(tmp_solution_previous_ut);
#else
        // If an older version of dealII is used, for example, 9.4.0,
        // interpolate() needs to use the following interface.
        solution_transfer_ut.interpolate(m_solution_previous_ut,
                                         tmp_solution_previous_ut);
#endif
        ...
        m_solution_previous_ut = tmp_solution_previous_ut;
    ...
    if (!mesh_is_same)
    {
      m_solution_ut = m_solution_previous_ut;
  ...
  template <int dim> void SplitSolveUTandD<dim>::run()
  {
    ...
    while (m_time.current() < m_time.end() - m_time.get_delta_t() * 1.0e-6)
    {
      ...
      m_solution_previous_ut = m_solution_ut;
           ...
           ++adp_refine_iteration)
      {
        ...
        BlockVector<double> solution_ut_prev_iter(m_dofs_per_block_ut);
        BlockVector<double> solution_ut_diff(m_dofs_per_block_ut);
             ...
             ++iter_stagger)
        {
          ...
          // staggered approach:
          // first, solve the thermomechanical subproblem (a nonlinear problem)
          if (m_parameters.m_output_iteration_history)
            m_logfile << "  \tsub-UT (nonlinear)" << std::flush;
          linear_solve_needed += displacement_thermal_step(iter_stagger);

          // then, solve the phase-field subproblem (a linear problem)
          if (m_parameters.m_output_iteration_history)
            m_logfile << "  sub-PF (linear)" << std::flush;
          phasefield_step();
          ++linear_solve_needed;

          // calculate the residual of the ut-subproblem
          assemble_rhs_ut();

          // calculate the residual of the phasefield-subproblem
          assemble_rhs_d();

          // calculate the displacement and temperature (ut) increment
          solution_ut_diff = m_solution_ut - solution_ut_prev_iter;
          ...
          for (unsigned int i = 0; i < m_dof_handler_ut.n_dofs(); ++i)
          {
            if (m_constraints_ut.is_constrained(i))
            {
              solution_ut_diff(i) = 0.0;
              m_system_rhs_ut(i) = 0.0;
            }
          }
          double ut_inc_l2 = solution_ut_diff.l2_norm();
          double ut_residual_l2 = m_system_rhs_ut.l2_norm();
          ...
          if (m_parameters.m_output_iteration_history)
          {
            m_logfile << "  " << ut_residual_l2 << "  " << d_residual_l2 << "  "
                      << ut_inc_l2 << "  " << d_inc_l2 << "  " << std::fixed
                      << std::setprecision(10) << std::scientific
                      << energy_functional_current << std::endl;
          }

          if ((iter_stagger > 1) &&
              (ut_residual_l2 < m_parameters.m_tol_ut_residual) &&
              (d_residual_l2 < m_parameters.m_tol_d_residual) &&
              (ut_inc_l2 < m_parameters.m_tol_ut_incr) &&
              (d_inc_l2 < m_parameters.m_tol_d_incr))
          {
            ...
            m_logfile << "\tStaggered approach (ut-d) converges after "
            ...
            m_logfile
                << "\t\tAbsolute residual of thermomechanical (ut) equations: "
                << ut_residual_l2 << std::endl;

            m_logfile << "\t\tAbsolute residual of phasefield (d) equation: "
                      << d_residual_l2 << std::endl;

            m_logfile
                << "\t\tAbsolute increment of displacement and temperature (ut): "
                << ut_inc_l2 << std::endl;

            m_logfile << "\t\tAbsolute increment of phasefield (d): " << d_inc_l2
                      << std::endl;
          ...
          {
            solution_ut_prev_iter = m_solution_ut;
        ...
        {
          m_logfile
              << "After " << m_parameters.m_max_staggered_iteration
              << " iterations, "
              << "no convergence is achieved in the ut-d staggered approach."
              << std::endl;
          AssertThrow(
              false,
              ExcMessage("No convergence achieved in ut-d staggered approach!"));
        }
...
int main(int argc, char *argv[])
{
  ...
  if (dim == 2)
  {
    PhaseField_uT_and_d::SplitSolveUTandD<2> Phasefield2D("parameters.prm");
    Phasefield2D.run();
  }
  else if (dim == 3)
  {
    PhaseField_uT_and_d::SplitSolveUTandD<3> Phasefield3D("parameters.prm");
    Phasefield3D.run();
  }
  else
  {
