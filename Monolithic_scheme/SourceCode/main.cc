...
namespace PhaseField_monolithic
{
  ...
  namespace Parameters
  {
    ...
    void Scenario::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Scenario");
      {
        ...
        prm.declare_entry("Preconditioner type for CG", "Jacobi",
                          Patterns::Selection("None|Jacobi|SSOR|ILU|AMG"),
                          "Type of preconditioner used to solve the linear system");

        prm.declare_entry("SSOR relaxation parameter", "1.2", Patterns::Double(0.0),
        ...
        prm.declare_entry("Local prerefinement times", "0", Patterns::Integer(0),
                          "Local pre-refinement times (assume crack path is known "
                          "a priori), only refine along the crack path.");

        prm.declare_entry("Max adaptive refinement times", "100",
                          Patterns::Integer(0),
                          "Maximum number of adaptive refinement times allowed "
                          "in each step");
        ...
        prm.declare_entry("Allowed max hl ratio", "0.25", Patterns::Double(),
                          "Allowed maximum ratio between mesh size h and "
                          "length scale l");
  ...
  private:
    ...
    void
    assemble_system_rhs_LBFGS_parallel(const BlockVector<double> &solution_old,
    ...
    double line_search_stepsize_residual_projection(
        const double f0, const BlockVector<double> &BFGS_p_vector,
        const BlockVector<double> &solution_delta);
  ...
  void PhaseFieldMonolithicSolve<dim>::get_error_update(
      const BlockVector<double> &soln_update, Errors &error_update)
  {
      ...
      const std::string &data_file, const unsigned int total_material_regions)
  {
    ...
    if (myfile.is_open())
    {
             ...
             reference_temperature >> max_temperature >> b_1 >> b_2)
      {
        ...
        AssertThrow(
            reference_temperature == m_parameters.m_ref_temperature,
  ...
  template <int dim> void PhaseFieldMonolithicSolve<dim>::setup_system()
  {
    ...
    m_logfile << "\t\tTriangulation:"
              ...
              << m_dofs_per_block[m_t_dof] << std::endl;
...
*/
  ...
  template <int dim> void PhaseFieldMonolithicSolve<dim>::assemble_system_B0()
  {
  ...
  template <int dim>
  void PhaseFieldMonolithicSolve<dim>::assemble_system_rhs_LBFGS_parallel(
      ...
      unsigned int max_iter, const BlockVector<double> &solution_delta)
  {
    ...
    for (; i < max_iter; ++i)
    {
      ...
      // phi_low_prime, 					alpha_high, phi_high,
      // phi_high_prime);
      current_phi_phi_prime =
          calculate_phi_and_phi_prime(alpha, BFGS_p_vector, solution_delta);
      phi = current_phi_phi_prime.first;
  ...
  template <int dim>
  double PhaseFieldMonolithicSolve<dim>::line_search_stepsize_residual_projection(
      const double f0, const BlockVector<double> &BFGS_p_vector,
      const BlockVector<double> &solution_delta)
  {
  ...
  template <int dim>
  double PhaseFieldMonolithicSolve<dim>::binary_search(
  ...
  template <int dim>
  double PhaseFieldMonolithicSolve<dim>::calculate_phi_prime(
      const double alpha, const BlockVector<double> &BFGS_p_vector,
      const BlockVector<double> &solution_delta)
  {
      ...
      */
      ...
      if (m_parameters.m_type_preconditioner == "None")
      {
        ...
        cg_tt.solve(m_tangent_matrix.block(m_t_dof, m_t_dof),
                    LBFGS_r_vector.block(m_t_dof), LBFGS_q_vector.block(m_t_dof),
                    PreconditionIdentity());
      }
      ...
      BlockVector<double> &LBFGS_update_refine)
  {
         ...
         ++LBFGS_iteration)
    {
      ...
      */
      ...
      // line_search_parameter =
      // line_search_stepsize_residual_projection(phi_0_prime,
      // LBFGS_r_vector,
      //								 solution_delta);
  ...
  template <int dim> void PhaseFieldMonolithicSolve<dim>::output_results() const
  {
    ...
    for (unsigned int i = 0; i < dim; ++i)
    {
      ...
      data_out.add_data_vector(dof_handler_L2, heat_flux_field_L2, heat_flux_name,
                               data_component_interpretation_L2);
      if (i == 0)
  ...
  void PhaseFieldMonolithicSolve<dim>::print_parameter_information()
  {
    ...
    if (m_parameters.m_type_linear_solver == "CG")
    {
      ...
      if (m_parameters.m_type_preconditioner == "AMG")
        m_logfile
            << "\t\tWARNING! In order to use the AMG preconditioner\n"
               "\t\tprovided by the PETScWrappers, vectors and matrices need to\n"
               "\t\tbe converted back and forth between the PETSc version and "
               "the\n"
               "\t\tdeal.ii version. As a result, there is a large overhead. The "
               "AMG\n"
               "\t\tpreconditioner is provided here only for demonstration "
               "purpose. For\n"
               "\t\tmore serious application, please use the MPI version of this "
               "code."
            << std::endl;
    }
...
int main(int argc, char *argv[])
{
  ...
  if (dim == 2)
  {
    PhaseField_monolithic::PhaseFieldMonolithicSolve<2> Phasefield2D(
        "parameters.prm");
    Phasefield2D.run();
  }
  else if (dim == 3)
  {
    PhaseField_monolithic::PhaseFieldMonolithicSolve<3> Phasefield3D(
        "parameters.prm");
    Phasefield3D.run();
  }
  else
